    ...
    for (i = 0; i < NUMBER_CATEGORIES; i++)
    {
      if(categories[i].clicked(mouseX, mouseY))
      {
        ...
        for(var k = 0; k < NUMBER_CATEGORIES; k++){
          if(k != i) {
            categories[k].changeType(UNSELECTED);
          }
        }
...
{
   //grupo de targets VE SE ISTO TMB SERVE
  // Colours
  WHITE = color(0,0,0);
  ...
  for(var i=0; i < num_targets_list.length; i++) {
    ...
    let target = new Target(target_x, target_y, width, height,target_label, label_id,col_id, j);
...
function createCategories(circle_size, screen_width, screen_height, big_circle_size, target_width, target_height) //ACRESCENTEI PARAMETROS
{
    ...
    for (var i = 0; i < NUMBER_CATEGORIES; i++)
    {
      ...
      createTargets(screen_width/2, screen_height/2, target_width, target_height); //CRIA OS TARGETS AQUI PARA SEREM ATINGIVEIS NO SKETCH
...
{    
  if (fullscreen())
  {
    ...
    createCategories(cat_size * PPCM, screen_width*PPCM, screen_height*PPCM, big_circle_size * PPCM, target_width *PPCM, target_height*PPCM); //ACRESCENTEI ALGUNS PARAMETROS
